...
open class FDWaveformView: UIView {
  ...
  open var progressPercent: CGFloat = 0.0 {
    didSet {
      guard totalSamples > 0,
            progressPercent <= 1.0,
            progressPercent >= 0.0 else {
        return
      }
      self.highlightedSamples = 0 ..< Int(CGFloat(totalSamples) * progressPercent)
    }
  }
  ...
  public func zoomToTimes(startTime: CMTime, endTime: CMTime, assetDuration: CMTime) {
    self.highlightedSamples = 0 ..< Int(CGFloat(totalSamples) * progressPercent)
    let duration = assetDuration.toSeconds()
    let startPercent = CGFloat(startTime.toSeconds() / duration)
    let startSample = Int(CGFloat(totalSamples) * startPercent)
    let endPercent = CGFloat(endTime.toSeconds() / duration)
    let endSample = Int(CGFloat(totalSamples) * endPercent)

    zoomSamples = (startSample ..< endSample)
  }
}
